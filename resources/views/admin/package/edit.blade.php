    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div class="p-6">
            <form action="{{ route('admin.package.update', $package->id) }}" method="POST" class="space-y-5">
                ...
                <div class="pt-4">
                    <button type="submit" class="w-full inline-flex justify-center items-center px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm rounded-md shadow-md transition-colors">
                        Update Package
                    </button>
                </div>
            </form>
...
<script>
    document.getElementById('add-custom-field').addEventListener('click', function() {
        ...
        row.querySelector('.field-type-selector').addEventListener('change', function(e) {
            ...
            if (type === 'text') valueContainer.innerHTML = `<input type="text" name="custom_fields[${index}][value]" placeholder="Enter text" class="w-full rounded-md border-gray-300 shadow-sm text-sm p-2" required>`;
