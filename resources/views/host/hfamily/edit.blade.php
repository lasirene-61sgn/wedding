            ...
            <form action="{{ route('host.hfamily.update', $hfamily->id) }}" method="POST">
                ...
                <div class="row g-3 mb-4 align-items-start border-bottom pb-3">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="topic_title_{{ $key }}" class="form-control" value="{{ old($titleField, $hfamily->$titleField) }}" placeholder="e.g., Groom's Parents">
                    </div>
                    <div class="col-md-8">
                        <label class="form-label fw-semibold text-dark">{{ $label }} Content Body / Names</label>
                        <textarea name="text{{ $key }}" class="form-control" rows="2" placeholder="List family members or relevant notes...">{{ old($textField, $hfamily->$textField) }}</textarea>
                    </div>
                </div>
                @endforeach
                ...
                <div class="mb-4">
                    ...
                    <textarea name="textseven" class="form-control" rows="3" placeholder="Any final matching invitation details...">{{ old('textseven', $hfamily->textseven) }}</textarea>
                </div>
